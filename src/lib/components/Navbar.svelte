...
<nav class="{scrollY > 48 ? `navbar-${theme} max-sm:backdrop-blur-xl` : ''} navbar-fade flex justify-between p-10 max-sm:pb-24 max-sm:fixed max-sm:top-0 max-sm:left-0 max-sm:right-0 max-sm:z-40 transition">
    ...
    <div class="flex gap-5">
        {#if isLoggedIn}
            <button on:click={() => (location.href = `${location.origin}/api/auth/logout`)}>
                <Power/>
            </button>
        {/if}
        {#if hasThemeToggle}
...
{#if mobileMenuOpen}
    ...
    >
        <div class="flex justify-between mb-10">
            <a on:click={() => mobileMenuOpen = false}
               class="logo-{theme} md:fixed top-6 md:border-b pb-1.5 pl-0.5 pr-8 -left-5 md:-rotate-90" href="/">
